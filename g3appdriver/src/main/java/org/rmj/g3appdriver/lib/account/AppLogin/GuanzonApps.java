...
public class GuanzonApps implements G3AppsLogin {
    ...
    public void loginUser(Context context, final HashMap headers, final JSONObject Parameters) {
        ...
        new HttpRequestUtil().sendRequest(new SharedPref(mContext).getAppServer() + "security/signin.php", new HttpRequestUtil.onServerResponseListener() {
            @Override
    ...
    private void saveLoginDataToLocal(JSONObject foValue){
        try {
            ...
            String sMobileNo = foValue.getString("sMobileNo");
            ...
            if (lnRow <= 0) {
                lsSQL = "INSERT INTO Client_Info_Master (   sUserIDxx, sEmailAdd, sUserName, dLoginxxx, sMobileNo, dDateMmbr)  VALUES (" + SQLUtil.toSQL(sUserIDxx) + ", " + SQLUtil.toSQL(sEmailAdd) + ", " + SQLUtil.toSQL(sUserName) + ", " + SQLUtil.toSQL(dLoginxxx) + ", "+SQLUtil.toSQL(sMobileNo)+", "+SQLUtil.toSQL(dCreatedx)+")";
